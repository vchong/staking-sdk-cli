...
# help(get_account_by_path)
# print("---")
# import inspect
# print(inspect.signature(get_account_by_path))
...
class LedgerSigner(Signer):
    ...
    def sign_transaction(self, tx: dict) -> SignedTransaction:
        ...
        ledger_tx = Type2Transaction(
            destination=bytes.fromhex(strip_0x(tx["to"])),
            amount=tx["value"],
            gas_limit=tx["gas"],
            data=bytes.fromhex(strip_0x(tx["data"])) if tx.get("data") else b"",
            nonce=tx["nonce"],
            chain_id=tx["chainId"],
        ...
        # eth_account >= 0.9.0 use raw_transaction (snake case)
        return SignedTransaction(
            raw_transaction=raw_tx,
            hash=Web3.keccak(raw_tx),
            r=signed.sender_r,
            s=signed.sender_s,
            v=signed.y_parity,
        )
